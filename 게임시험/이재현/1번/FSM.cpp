...
State* Fsm::CreateState(STATE_TYPE state)
{
    ...
    switch (state)
    {
    case STATE_IDLE:   return new IdleState(this);
    case STATE_MOVE:   return new MoveState(this);
    case STATE_ATTACK: return new AttackState(this);
    default:
        return nullptr;
    }//필요에 따라 변경 가능
}
...
void MoveState::Update()
{
    ...
    //응시자 코드 작성 부분
    auto inputs = _fsm->GetInputs();
    if (inputs.empty()) { _fsm->Transition(STATE_IDLE);  return; }
    
    if (inputs.find(INPUT_ATTACK) != inputs.end()) { _fsm->Transition(STATE_ATTACK);  return; }
...
void AttackState::Update()
{
    ...
    if ((time - _start) > 500) { _fsm->Transition(STATE_IDLE);  return; }
    //상태 변경이 되지 않을경우 실행 (변경하지 마시오.)
